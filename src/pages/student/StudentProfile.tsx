...
const StudentProfile: React.FC = () => {
  ...
  return (
    <UserLayout role="STUDENT" pageTitle="My Profile" activePath="/student-profile">
      <div className="space-y-6">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-5">
            ...
            {!editing ? (
              <button onClick={() => setEditing(true)} className="flex items-center gap-1.5 text-xs font-semibold py-2 px-3.5 rounded-xl transition cursor-pointer" style={{ background: 'var(--role-bg)', color: 'var(--role-accent)', border: '1px solid var(--role-border)' }}>
                <Pencil size={14} /> Edit
              </button>
            ) : (
              <div className="flex items-center gap-2">
                <button onClick={handleCancel} disabled={saving} className="flex items-center gap-1.5 text-xs font-semibold py-2 px-3.5 rounded-xl bg-slate-800 text-slate-300 border border-slate-700 hover:bg-slate-700 transition cursor-pointer disabled:opacity-50">
                  <X size={14} /> Cancel
                </button>
                <button onClick={handleSave} disabled={saving} className="flex items-center gap-1.5 text-xs font-semibold py-2 px-3.5 rounded-xl text-white transition cursor-pointer disabled:opacity-50" style={{ background: 'var(--role-primary)' }}>
            ...
            {profileFields.map((field) => (
              <div key={field.key} className="flex items-start gap-3 p-3.5 rounded-xl bg-slate-800/50 border border-slate-700/50">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {editing ? (
                    field.type === 'select' ? (
                      <select value={form[field.key as keyof typeof form]} onChange={(e) => handleChange(field.key, e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-100 outline-none focus:border-indigo-500 transition appearance-none cursor-pointer">
                      ...
                      <input type={field.type} value={form[field.key as keyof typeof form]} onChange={(e) => handleChange(field.key, e.target.value)} placeholder={`Enter ${field.label.toLowerCase()}`} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-100 placeholder-slate-600 outline-none focus:border-indigo-500 transition" />
                    )
                  ) : (
                    <p className="text-sm font-medium text-white truncate">
                      {field.type === 'date' ? formatDate(form[field.key as keyof typeof form]) : (form[field.key as keyof typeof form] || '—')}
                    </p>
                  )}
                </div>
              </div>
